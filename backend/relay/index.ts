...
serve(async (req: Request) => {
  ...
  if (action === "enroll") {
    ...
    try {
      ...
      await conn.queryObject(
        `INSERT INTO public.enrollment_queue (finger_id, user_name, uen_number, gender, bus_driver, bus_number, status)
         VALUES ($1, $2, $3, $4, $5, $6, 'pending')
         ON CONFLICT (finger_id) DO UPDATE
           SET user_name=$2, uen_number=$3, gender=$4, bus_driver=$5, bus_number=$6, status='pending'`,
        [fingerId, userName, uenNumber, gender, busDriver, busNumber]
      );
  ...
  if (action === "users") {
    ...
    try {
      const result = await conn.queryObject(
        `SELECT id, finger_id, name, uen_number, gender, bus_driver, bus_number, created_at
  ...
  if (params.has("FingerID")) {
    ...
    const conn = await pool.connect();
    try {
      const userRes = await conn.queryObject<{name:string,uen_number:string,gender:string,bus_driver:string,bus_number:string}>(
        `SELECT name, uen_number, gender, bus_driver, bus_number FROM public.users WHERE finger_id=$1`, [fingerId]
      );
      ...
      await conn.queryObject(
        `INSERT INTO public.attendance
           (finger_id, user_name, uen_number, gender, bus_driver, bus_number, event_type, latitude, longitude)
         VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9)`,
        [fingerId, user.name, user.uen_number, user.gender, user.bus_driver, user.bus_number, eventType, lat, lng]
      );
  ...
  if (params.has("confirm_id")) {
    ...
    const conn = await pool.connect();
    try {
      const qRes = await conn.queryObject<{finger_id:number,user_name:string,uen_number:string,gender:string,bus_driver:string,bus_number:string}>(
        `SELECT * FROM public.enrollment_queue WHERE finger_id=$1 AND status='pending' LIMIT 1`, [fingerId]
      );
      if (qRes.rows.length) {
        const r = qRes.rows[0];
        await conn.queryObject(
          `INSERT INTO public.users (finger_id, name, uen_number, gender, bus_driver, bus_number)
           VALUES ($1,$2,$3,$4,$5,$6)
           ON CONFLICT (finger_id) DO UPDATE
             SET name=$2, uen_number=$3, gender=$4, bus_driver=$5, bus_number=$6`,
          [r.finger_id, r.user_name, r.uen_number, r.gender, r.bus_driver, r.bus_number]
        );
